# -*- coding: utf-8 -*-
"""
並行回測執行器：利用多核心 CPU 同步跑多檔股票 point-in-time 回測，
完成後合併結果並調用 analyze_results.py 輸出驗證報告。
"""
import os
import sys
import time
...
def run_worker(ticker, months=12, step=5, start_date="2020-01-01"):
    out_file = f"F:\\stock\\backtest\\part_{ticker}.csv"
    try:
...
def main():
    ...
    merged_output = r"F:\stock\backtest\signals_modified_m12_s5.csv"

    print(f"=== 啟動多核心並行回測 ({len(TARGET_TICKERS)} 檔個股，step={step}，months={months}) ===")
    t0 = time.time()

    workers = min(len(TARGET_TICKERS), 9)
    dfs = []
