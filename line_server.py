...
SYNC_SECRET_KEY = os.environ.get("SYNC_SECRET_KEY", "").strip() or (CHANNEL_SECRET if CHANNEL_SECRET != "YOUR_CHANNEL_SECRET" else "")
...
def _get_sync_headers():
    """產出同步金鑰標頭，防止未授權存取資料庫快照"""
    token = os.environ.get("SYNC_SECRET_KEY", "").strip() or (CHANNEL_SECRET if CHANNEL_SECRET != "YOUR_CHANNEL_SECRET" else "")
    return {"X-Sync-Token": token} if token else {}


def _verify_sync_token(req: Request) -> bool:
    """驗證快照端點的認證權限"""
    expected_token = os.environ.get("SYNC_SECRET_KEY", "").strip() or (CHANNEL_SECRET if CHANNEL_SECRET != "YOUR_CHANNEL_SECRET" else "")
    if not expected_token:
        # 若兩端環境均未配置任何 Secret，則允許直通
        return True

    token = req.headers.get("X-Sync-Token", "").strip()
    if not token:
...
def push_snapshot_to_cloud(max_retries: int = 3):
    """（同步）將本地資料庫快照推送到 Render 雲端備援，支援指數退避重試"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            res = requests.post(f"{RENDER_BACKEND_URL}/api/sync_db", json=snapshot, headers=headers, timeout=(5, 15))
            if res.status_code == 200:
                logger.info("✅ 已成功將本地持倉與自選資料庫同步至 Render 雲端！")
                return True
            elif res.status_code == 401:
                logger.error("❌ 推送快照至雲端被拒：金鑰驗證失敗 (HTTP 401 Unauthorized)，請檢查 SYNC_SECRET_KEY 設定")
                return False
            else:
                logger.warning(f"推送快照至雲端被拒：HTTP {res.status_code}（嘗試 {attempt}/{max_retries}）")
        except Exception as e:
            logger.warning(f"雲端資料庫推送異常（嘗試 {attempt}/{max_retries}）: {e}")
...
def pull_snapshot_from_cloud():
    ...
    try:
        ...
        res = requests.get(f"{RENDER_BACKEND_URL}/api/sync_db", headers=headers, timeout=(5, 40))
        if res.status_code == 401:
            logger.error("❌ 自雲端拉回快照失敗：金鑰驗證失敗 (HTTP 401 Unauthorized)")
            return False
        if res.status_code != 200:
...
# ── 記憶體快取機制（使用 cachetools.TTLCache，具容量上限、自動過期與執行緒安全保護） ──
_STOCK_CACHE = TTLCache(maxsize=128, ttl=60)
_STOCK_CACHE_LOCK = threading.Lock()

_STOCK_5M_CACHE = TTLCache(maxsize=128, ttl=30)
_STOCK_5M_CACHE_LOCK = threading.Lock()

def get_cached_stock_analysis(ticker: str, months: int = 12, quick_mode: bool = False):
    """
    獲取個股多維度量化數據，支援 60s 執行緒安全 TTLCache
    """
    ticker_clean = str(ticker).strip().upper()
    full_key = f"{ticker_clean}_{months}_False"
    cache_key = f"{ticker_clean}_{months}_{quick_mode}"

    with _STOCK_CACHE_LOCK:
        if full_key in _STOCK_CACHE:
            logger.info(f"快取命中！【{ticker_clean}】(完整快取) 自記憶體即刻回傳（耗時 < 1ms）")
            return _STOCK_CACHE[full_key]
        if cache_key in _STOCK_CACHE:
            logger.info(f"快取命中！【{ticker_clean}】(quick_mode={quick_mode}) 自記憶體即刻回傳（耗時 < 1ms）")
            return _STOCK_CACHE[cache_key]

    res = analyze_stock(ticker_clean, months=months, generate_html=False, print_report=False, quick_mode=quick_mode)
...
@app.get("/api/sync_db")
def get_db_snapshot(req: Request = None):
    """供雙節點查詢資料庫快照（具備 Token 驗證）"""
    if req is not None and not _verify_sync_token(req):
...
async def post_db_snapshot(req: Request):
    """供雙節點寫入資料庫快照（具備 Token 驗證、大小限制與格式檢查）"""
    if not _verify_sync_token(req):
        return JSONResponse({"status": "error", "detail": "Unauthorized"}, status_code=401)

    content_length = req.headers.get("content-length")
    if content_length and int(content_length) > 10 * 1024 * 1024:
        return JSONResponse({"status": "error", "detail": "Payload too large (>10MB)"}, status_code=413)

    try:
        data = await req.json()
        if not isinstance(data, dict):
            return JSONResponse({"status": "error", "detail": "Invalid snapshot format (expected JSON object)"}, status_code=400)
...
def _cmd_portfolio(user_id: str, user_name: str):
    ...
    return bot_flex.build_portfolio_flex(user_name, items, total_pnl, total_pnl_pct)


def _cmd_add_watchlist(user_id: str, cmd: str, tokens: list) -> str:
    """加入自選觀察清單"""
    if cmd.startswith("+"):
...
def _cmd_remove_watchlist(user_id: str, cmd: str, tokens: list) -> str:
    """取消關注 / 移出觀察名單"""
    if cmd.startswith("-"):
        raw_t = cmd.lstrip("-").strip()
    else:
...
def _cmd_view_watchlist(user_id: str, user_name: str):
    """查詢自選觀察清單"""
...
# ── 主指令分派器 (Main Command Dispatcher) ──────────────────
...
def handle_user_command(user_id: str, text: str, user_name: str = "投資人", is_group: bool = False):
    ...
    if action in ["買", "買進", "進場", "add", "buy"]:
        return _cmd_buy(user_id, tokens)
